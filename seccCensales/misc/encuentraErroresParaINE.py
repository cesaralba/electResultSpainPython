...
def checks2str(resChecks, colClave=None):
    ...
    cellProblems = lambda: {
        "valorMayoritario": defaultdict(lambda: defaultdict(set)),
    ...
    for res in resChecks:
        for comb in res["combs"]:
            if "valorMayoria" in comb:
                valMay = (
                    comb["valorMayoria"][0]
                    if isinstance(comb["valorMayoria"], list)
                    else comb["valorMayoria"]
                )
                aux = (
                    comb["divergentes"]
                    .apply(infoSC, axis=1, comb=comb, clave=colClave)
                    .tolist()
                )
                for name, IDX, valIDX, VAL, valVAL in aux:
                    secProblematicas[name][VAL]["valorUsado"] = valVAL
                    secProblematicas[name][VAL]["valorMayoritario"][valMay][IDX].add(
                        valIDX
                    )
                    divergentes[name] = comb["divergentes"]
            else:
                nombres = (
                    comb["divergentes"].index.to_list()
                    if colClave is None
                    else comb["divergentes"][colClave].to_list()
                )
                ...
                secEmpate[clave][comb["claveVAL"]]["fuentes"][comb["claveIDX"]].add(
                    comb["valorIDX"]
                )
    ...
    for probSEC in secProblematicas:
        ...
        for clave, valor in secProblematicas[probSEC].items():
            ...
            for valorMay, usadoEN in valor["valorMayoritario"].items():
                cadenasEN = [
                    f"{claveEN}=({','.join(usosEN)})"
                    for claveEN, usosEN in usadoEN.items()
                ]
