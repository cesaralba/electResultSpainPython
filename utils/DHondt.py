...
def DHondt(
    fila,
    votos="votCand",
    numescs=("datosTerr", "numEscs"),
    votBlanco=("datosTerr", "votBlanco"),
    umbral=0.0,
    calculaCosteAsiento=False,
    calculaUltimoElecto=False,
    calculaVotosSinEsc=False,
    ...
    if isinstance(fila, pd.core.series.Series):
        ...
        if isinstance(numescs, (int, np.uint32, np.uint64)):
            ...
            print(
                "DHondt: clave '%s' para numero de escaños no está en fila." % numescs
            )
            return None
        ...
        if isinstance(votBlanco, (int, np.uint32, np.uint64)):
            ...
            print(
                "DHondt: clave '%s' para numero de votos en blanco no está en fila."
                % votBlanco
            )
            return None
    ...
    if sum(actVotos) > 0:
        cocientes = [
            (x, y + 1, umbVotos[x] / (y + 1), umbVotos[x])
            for x in umbVotos.index
            for y in range(VnumEscs)
        ]
    ...
    elegidos.index = pd.MultiIndex.from_tuples(
        [("asignados", x) for x in elegidos.index]
    )
    ...
    if calculaCosteAsiento:
        costeAsiento = {
            ("costeAsiento", x): umbVotos[x] / dictElegidos[x]
            for x in dictElegidos
            if dictElegidos.get(x, 0) > 0
        }
    ...
    if calculaUltimoElecto:
        ...
        if ultEleg is None:
            ultimoSi = pd.Series(
                {("ultElegido", "Partido"): None, ("ultElegido", "Posicion"): None}
            )
        else:
            ultimoSi = pd.Series(
                {
                    ("ultElegido", "Partido"): ultEleg[0],
                    ("ultElegido", "Posicion"): ultEleg[1],
                }
            )
        ...
        if primNoEleg is None:
            primeroNo = pd.Series(
                {
                    ("primNoElegido", "Partido"): None,
                    ("primNoElegido", "Posicion"): None,
                }
            )
            ...
            primeroNo = pd.Series(
                {
                    ("primNoElegido", "Partido"): primNoEleg[0],
                    ("primNoElegido", "Posicion"): primNoEleg[1],
                }
            )
    ...
    if calculaVotosSinEsc:
        votosSinEsc = {
            ("votosSinAsiento", x): actVotos[x]
            if (x not in dictElegidos or dictElegidos[x] == 0)
            else 0
            for x in actVotos.index
        }
    ...
    if calculaCortadosUmbral:
        ...
        votosUmbral = pd.Series(
            {
                ("votosUmbral", "pasa"): umbVotos.sum(),
                ("votosUmbral", "noPasa"): noPasaUmbral,
            },
            dtype=np.uint64,
        )
